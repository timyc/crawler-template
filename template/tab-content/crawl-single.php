    ...
    <script type="text/javascript">
        $ = jQuery;
            ...
            $(document).on( 'click', '#crawl-single-manga', function(e){
                ...
                if( mangaURL.indexOf('http://{SITEURL}/manga') !== -1 || mangaURL.indexOf('https://{SITEURL}/manga') !== -1 ){
                    $.ajax({
                        method : 'POST',
                        url : "<?php echo esc_url( $ajax_url ); ?>",
                        data : {
                            action : '{NICKLOWER}_create_manga_post',
                            url : mangaURL
                        },
                        beforeSend: function(){
                            progressing.show();
            ...
            function fetchChapterLists( postID, mangaURL ){
                ...
                $.ajax({
                    ...
                    url : "<?php echo esc_url( $ajax_url ); ?>",
                    data : {
                        action: '{NICKLOWER}_fetch_chapters',
                        url:    mangaURL,
                        postID: postID
                    },
                    beforeSend : function(){
            ...
            function fetchSingleChapter( postID, data, vIndex, cIndex ){
				...
				if(data[ vIndex ].chapters[ cIndex ]){
					...
					$.ajax({
						...
						timeout: 250000,
						data : {
							action : '{NICKLOWER}_fetch_single_chapter',
							chapter : chapter,
						...
						success : function( response ){
							if( response.success ){
								...
								if( cIndex + 1 !== data[ vIndex ].chapters.length  ){ // If this is not latest chapter in volume
									...
									<?php if( class_exists('{NICK}_CRAWLER_CRONJOB') ){ ?>
										...
										<?php
											...
											if( $settings['import']['storage'] !== 'local' ){
												$cronjob = new {NICK}_CRAWLER_CRONJOB();
												?>
												// Upload cloud action
												$.ajax({
													method : 'POST',
            ...
            function removeInserted( postID ){
                $.ajax({
                    ...
                    url : "<?php echo esc_url( $ajax_url ); ?>",
                    data : {
                        action : '{NICKLOWER}_remove_inserted',
                        postID : postID
                    },
                });
            }
