<?php
    ...
    if( ! class_exists('WP_CRAWLER_ACTIONS') && class_exists('WP_CRAWLER_IMPORT') ){

        class WP_CRAWLER_ACTIONS extends WP_CRAWLER_IMPORT{
            ...
            */
            public function fetch_manga(){
				if(!get_transient('{NICKLOWER}_fetch_manga_running')){
					set_transient('{NICKLOWER}_fetch_manga_running', 1, 20 * 60); 
					$queue_list = $this->get_queue_list( 6 );
					if( !empty( $queue_list ) ){

						$queue_list = array_values( $queue_list );

						crawler_log( '.Start Fetching Manga' );
						...
						crawler_log( '.Done Fetching Manga' );
					}
					
					delete_transient('{NICKLOWER}_fetch_manga_running');
				} else {
					crawler_log('Another Fetch Manga process running');
				}
            }
            ...
            */
            public function fetch_single_manga( $manga ){
                ...
                if( !empty( $manga['is_update'] ) ){
					crawler_log('Update Manga: ' . $manga['name']);
                    if( !empty( $manga['post_id'] ) ){
                        $post_id = $manga['post_id'];
                    }else{
                        // Get the manga post
                        $post_id = find_manga_post( strtolower(html_entity_decode($manga['name']) ) );
                    }
                } else {
					crawler_log('get_manga_post_by_import_slug: ' . $manga['slug']);
                    $post_id = get_manga_post_by_import_slug( $manga['slug'], strtolower(html_entity_decode($manga['name']) ) );
                }

                // If no post found then create it
                if( empty( $post_id ) ){
					crawler_log('Cannot find existing manga: ' . $manga['name']);
					
					if(get_transient('{SITENAME}_crawler_creating_manga')){
						crawler_log('Another process is running. Waiting...');
						return true;
					} else {
						// flag to make sure we do not create duplicated manga
						set_transient('{SITENAME}_crawler_creating_manga', $manga['name'], 120);
						
						$post_id = $this->create_manga( $manga );
						
						delete_transient('{SITENAME}_crawler_creating_manga');

						// If manga is blocked by country
						if( !$post_id || (is_wp_error( $post_id ) && $post_id->get_error_code() == 'blocked') ){
					...
					crawler_log('Existing Manga: ' . $post_id);
				}
				
				$manga_title = get_the_title( $post_id );

                crawler_log( '..Fetching Manga <strong>' . $manga_title . '</strong>' );
                ...
                if(
                    ...
                    $html = $this->get_site_html( $manga['url'] );
                    ...
                    crawler_log( '...Fetch <strong>' . $manga_title . '</strong> Chapters list' );
                    ...
                    crawler_log( $resp ? '....Fetched Chapters list successfully' : '....Fetched chapters list failed' );
            ...
            */
            protected function update_latest_manga(){
                ...
                crawler_log( '.....Get Update Manga List that released yesterday' );
				// we crawl latest 8 pages
				for($i = 1; $i++; $i <= 8){
					$html = $this->get_site_html( $this->manga_listing_latest_url . ($i == 1 ? '' : $i . '.html') );
					...
					if( $mangas ){
						...
						foreach( $mangas as $manga ){
							...
							// Find manga post
							$manga_post_id = find_manga_post( $a_tag->title );
                ...
                crawler_log( '.....Done Get Update Manga List' );
            ...
            public function get_update_log( $day ){
            ...
            protected function put_update_log( $chapter ){
                ...
                $update = $this->get_update_log( $day );
            ...
            */
            ...
            public function upload_cloud(){
				if( ! class_exists('WP_MANGA_STORAGE') ) {
					crawler_log('WP Manga Core is missing');
				...
				if( get_transient( "{$this->fname}_is_upload_cloud_running" ) ){
					if(time() > get_option("_transient_timeout_{$this->fname}_is_upload_cloud_running")){
						crawler_log('Another Upload Cloud is running. Waiting');
                ...
                // Use for remove in error_log_die func
                ...
                if( !empty( $_POST['postID'] ) ){
                    ...
                    foreach( $upload_list as $id => $c ){
						...
						if(!$existing_chapter || $existing_chapter['volume_id'] != $chapter_args['volume_id']){
							set_transient('{SITENAME}_crawler_cloud_uploading', $id, 3600);
							crawler_log('Upload to Cloud: ' . $id);
							$result = $wp_manga_storage->wp_manga_upload_single_chapter( $chapter_args, $extract, $extract_uri, $storage );
							
							delete_transient('{SITENAME}_crawler_cloud_uploading');

							if( ! $result ) {
								crawler_log('upload to cloud failed. Please check this chapter: ' . $c['chapter_args']['chapter_name'] . ' of manga: ' . $c['chapter_args']['post_id']);
								
								crawler_log('As uploading to cloud failed, we upload to Local to bypass');
								$result = $wp_manga_storage->wp_manga_upload_single_chapter( $chapter_args, $extract, $extract_uri, 'local' );
								crawler_log('Upload Local done: ' . var_export($result, true));
								if(!$result){
									crawler_log('Upload Local failed, we bypass this chapter. You can do it manually later');
									$result = true;
								}
								
							} else {
								// delete local folder
								crawler_log('Delete local folder: ' . $extract);
								{NICKLOWER}_delete_files($extract);
							}
						}
            ...
            public function crawl(){
                ...
                $GLOBALS['crawler_log_dir'] = $this->data_dir;
                ...
                crawler_log( '<strong>Crawler Finish</strong>' );
