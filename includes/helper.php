<?php
	 ...
	 */
    if( ! function_exists( 'wp_mcl_e_upload_file_handler' ) ){

        function wp_mcl_e_upload_file_handler(){
        ...
        add_action('wp', 'wp_mcl_e_upload_file_handler');

    }

    if( ! function_exists('wp_mcl_e_upload_file') ){
        function wp_mcl_e_upload_file( $url, $post_id = 0 ){
    ...
    if( !function_exists('get_site_html') ){
        function get_site_html( $url ){
    ...
    if( ! function_exists('get_manga_post_by_import_slug') ){
        function get_manga_post_by_import_slug( $slug = '', $name = '' ){
    ...
    if( ! function_exists('find_manga_post') ){
        function find_manga_post( $name, $slug = '' ){

            if( !empty( $slug ) ){
                $post_id = get_manga_post_by_import_slug( $slug, $name );
            }
    ...
    if( ! function_exists( 'error_log_die' ) ){
        function error_log_die( $args ){
            ...
            $log_dir = "{$upload_dir['basedir']}/wp-crawler-cronjob/{SITENAME}-manga-crawler";
    ...
    if( ! function_exists( 'crawler_log' ) ){
        function crawler_log( $message ){
            // write plugin own crawler log
			$upload_dir = wp_get_upload_dir();
            $dir = "{$upload_dir['basedir']}/wp-crawler-cronjob/{SITENAME}-manga-crawler/crawler-log/";
    ...
    if( ! function_exists( 'get_crawler_log' ) ){
        function get_crawler_log( $dir, $date ){
    ...
    if( ! function_exists( 'check_curl_proxy' ) ){
        function check_curl_proxy( $proxy ){
            ...
            $resp = curl_get_contents( $url, true );
	 ...
	 */
	function {NICKLOWER}_delete_files($target) {
		if(is_dir($target)){
			$files = glob( $target . '*', GLOB_MARK ); //GLOB_MARK adds a slash to directories returned

			foreach( $files as $file ){
				{NICKLOWER}_delete_files( $file );      
			}
