<?php
    ...
    class {NICK}_CRAWLER_SINGLE extends {NICK}_CRAWLER_IMPLEMENT{

        public function __construct(){
            parent::__construct();

			add_action( 'wp_ajax_{NICKLOWER}_create_manga_post', array( $this, 'create_manga_post' ) );
            add_action( 'wp_ajax_{NICKLOWER}_fetch_chapters', array( $this, 'fetch_manga_chapters' ) );
            add_action( 'wp_ajax_{NICKLOWER}_fetch_single_chapter', array( $this, 'fetch_manga_single_chapter' ) );
            add_action( 'wp_ajax_{NICKLOWER}_remove_inserted', array( $this, 'remove_inserted_manga' ) );
		...
		public function create_manga_post( $url = ''){
			...
			$post_id = find_manga_post( $manga_name );
			
			if(!$post_id){
				$post_id = get_manga_post_by_import_slug( $this->manga_slug_filter($url), $manga_name );
			}
        ...
        public function fetch_manga_chapters(){
            ...
            $html = $this->get_site_html( $_POST['url'] );
