    ...
    <table class="form-table">
        <tbody>
            <tr>
                ...
                <td>
                    <span>
                        <?php if( !empty( ${NICKLOWER}_crawler_settings['status']['next_update_latest_manga'] ) ){ ?>
                            <?php echo date_i18n( 'Y-m-d H:i:s', ${NICKLOWER}_crawler_settings['status']['next_update_latest_manga'] ); ?>
                            -
                            <?php echo human_time_diff( time(), ${NICKLOWER}_crawler_settings['status']['next_update_latest_manga'] ) . esc_html__( ' left', WP_MCL_TD ); ?>
                        <?php } ?>
                    </span>
                </td>
            </tr>
    ...
    jQuery(document).ready(function($){
        ...
        $('#log-date').on('change', function(){
            ...
            $.ajax({
                ...
                method : 'GET',
                data : {
                    action : '{NICKLOWER}_get_update_log',
                    date : date
                },
                beforeSend : function(){
