<?php
    ...
    if( ! class_exists( 'WP_CRAWLER_IMPORT' ) ){

        class WP_CRAWLER_IMPORT{
            ...
            protected function fetch_chapters( $chapter_list, $post_id, $is_update ){
                ...
                crawler_log( '....Current Volume : ' . $volume['name'] );

                if( empty( $volume['chapters'] ) ){
                    // If this chapter doesn't exist then this is last chapter, and update volume
                    crawler_log( "....{$volume['name']} doesn't have any chapter, go to next volume" );
					...
					if(isset($all_vols[$index+1])){
						...
						crawler_log( ".....Cannot find any chapters and no more volume. Exit." );
						return false;
					}
                ...
                if( isset( $cur_chap_index ) ){
                    ...
                    crawler_log( ".....Current Chapter : {$cur_chap['name']}" );
					...
					crawler_log('Fetch single chapter result: ' . var_export($resp, true));
            ...
            public function fetch_single_chapter( $chapter, $volume_id, $post_id ){
                ...
                foreach( $chapter_images as $image ){
                    if($idx > $existing_images){
                        ...
                        $file_name = $this->url_file_name_filter( $pathinfo['basename'] );
            ...
            protected function get_page_images( $page_url ){
                ...
                $html = $this->get_site_html( $page_url );
            ...
            protected function get_manga_name( $html ){
                ...
                return !empty( $name ) ? $name[0]->plaintext : '';
            ...
            public function get_manga_thumb( $html ){
                ...
                $thumb = !empty( $thumb ) ? $thumb[0]->src : null;
            ...
            private function upload_featured_image( $thumb_url, $post_id ){
                ...
                if( !empty( $thumb_url ) ){
                    return wp_mcl_e_upload_file( $thumb_url, $post_id );
                }
